# Введение
Это приложение предназначено для структурирования информации в виде графов понятий - mind map (иногда называемых "интеллект-карты", к сожалению, адекватного перевода этого понятия на русский не существует).

Данное приложение отличается от подобных, во-первых, наличием расширенных возможностей использования mind maps для обучения, во-вторых — способностью взаимодействовать с нейронными сетями.

Для целей обучения используются тесты и статусы "пройдено" у узлов. Используемый формат данных, в свою очередь, упрощает взаимодействие с нейросетями (LLM) - это простой JSON с подробной спецификацией.

Также нужно отметить, несмотря на работу в браузере, это офлайн-приложение: после запуска ему не нужен доступ в Интернет, данные сохраняются локально.
# Работа со схемой
